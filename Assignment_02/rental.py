    ...
    def __init__(self, name, license_no):
        ...
        def license_no(self, value):
            if not value or value <= 0:
                raise ValueError('license_no must be empty or a positive number')
...
class ElectricCar(Vehicle):
    ...
    def __str__(self):
        status = 'rented' if self.is_rented else 'available'
        return f'{self.make} {self.model} ({self.plate}) - {status} [electric: {self.battery_kwh} kWh]'
...
class Motorbike(Vehicle):
    ...
    def __str__(self):
        status = 'rented' if self.is_rented else 'available'
        return f'{self.make} {self.model} ({self.plate}) - {status} [motorbike: {self.engine_cc} cc]'